...
DepthPeeling::DepthPeeling(DX::Graphic* graphic)
	//:m_dxGraphic(graphic)
{
	...
	m_renderQuad->transform->SetRot(-DIR_FORWARD, DIR_UP);
	...
	m_renderQuad->ps->AddSRV(SHADER_REG_SRV_TEX, 1);*/
}
...
/*
	...
	for (int i = 0; i < nPic; ++i)
	{
		...
		m_dxPShader->WriteSRV(10, (i == 0) ? nullSRV : m_vdxPic[i - 1].depthSrv);
		...
		for (auto it = objs.begin(); it != objs.end(); ++it)
		{
			...
			const SHADER_STD_TRANSF STransformation(obj->transform->WorldMatrix(), v, p);
			...
			m_dxPShader->WriteSRV(SHADER_REG_SRV_TEX, mainTex);
...
void DX::DepthPeeling::Render(ID3D11DeviceContext* dContext, int iIndex)
{
	...
	if (iIndex < 0)
	{
		...
		{
			m_renderQuad->ps->WriteSRV(SHADER_REG_SRV_TEX, it->backSrv);
			m_renderQuad->Render(m_dxGraphic->DContext(), XMMatrixIdentity(), XMMatrixIdentity(), nullptr);
		}
	}
	else
	{
		m_renderQuad->ps->WriteSRV(SHADER_REG_SRV_TEX, m_vdxPic[iIndex].backSrv);
		m_renderQuad->Render(m_dxGraphic->DContext(), XMMatrixIdentity(), XMMatrixIdentity(), nullptr);
	}
}
*/
