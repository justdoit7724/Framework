...
bool CubeCollider::IsHit(Ray ray, XMFLOAT3* hitPt)
{
	XMFLOAT3 right = Cross(up, forward);

	if (Dot(ray.d, right) < 0)
	{
		Plane rightP(center + right * extent.x, right, up, XMFLOAT2(extent.z, extent.y));

		if (IntersectRayPlane(ray, rightP, hitPt))
			return true;
	}
	if (Dot(ray.d, up) < 0)
	{
		Plane upP(center + up * extent.y, up, -forward, XMFLOAT2(extent.x, extent.z));
	...
	{
		Plane forwardP(center + forward * extent.z, forward, up, XMFLOAT2(extent.x, extent.y));
	...
	if (Dot(ray.d, -right) < 0)
	{
		Plane leftP(center - right * extent.x, -right, up, XMFLOAT2(extent.z, extent.y));
	...
	if (Dot(ray.d, -up) < 0)
	{
		Plane downP(center - up * extent.y, -up, forward, XMFLOAT2(extent.x, extent.z));
	...
	if (Dot(ray.d, -forward) < 0)
	{
		Plane backP(center - forward * extent.z, -forward, up, XMFLOAT2(extent.x, extent.y));
